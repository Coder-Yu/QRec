...
class GACL(GraphRecommender):
    ...
    def readConfiguration(self):
        ...
        args = config.LineConfig(self.config['GACL'])
        self.ssl_rate = float(args['-lambda'])
    ...
    def initModel(self):
        ...
        n_ego_embeddings1 = ego_embeddings + tf.multiply(tf.math.sign(ego_embeddings),tf.nn.l2_normalize(random_noise, 1) * self.eps)
        random_noise = tf.random.uniform(ego_embeddings.shape)
        n_ego_embeddings2 = ego_embeddings + tf.multiply(tf.math.sign(ego_embeddings),tf.nn.l2_normalize(random_noise, 1) * self.eps)
        #adjaceny matrix
        ...
        for k in range(self.n_layers):
            ...
            n_ego_embeddings1 +=  tf.multiply(tf.math.sign(n_ego_embeddings1),tf.nn.l2_normalize(random_noise, 1) * self.eps)
            random_noise = tf.random.uniform(n_ego_embeddings2.shape)
            n_ego_embeddings2 += tf.multiply(tf.math.sign(n_ego_embeddings2), tf.nn.l2_normalize(random_noise, 1) * self.eps)
    ...
    def buildModel(self):
        ...
        rec_loss = -tf.reduce_sum(tf.log(tf.sigmoid(y))) + self.regU * (tf.nn.l2_loss(self.batch_user_emb) + tf.nn.l2_loss(self.batch_pos_item_emb) + tf.nn.l2_loss(self.batch_neg_item_emb))
        loss = rec_loss+self.ssl_loss
        ...
        for epoch in range(self.maxEpoch):
            for n, batch in enumerate(self.next_batch_pairwise()):
                ...
                _, l, rec_l, ssl_l = self.sess.run([train, loss, rec_loss, self.ssl_loss],
