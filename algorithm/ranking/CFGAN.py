...
class CFGAN(DeepRecommender):
    ...
    def next_batch(self):
        ...
        N_u = np.zeros((self.batch_size,self.num_items))
    ...
    def initModel(self):
        ...
        with tf.variable_scope("Generator"):
            ...
            G_W1 = tf.get_variable(name='G_W1',initializer=xavier_init([self.num_items,300]), regularizer=G_regularizer)
        ...
        with tf.variable_scope("Discriminator"):
            ...
            D_W1 = tf.get_variable(initializer=xavier_init([self.num_items,300]), name='D_W1',regularizer=D_regularizer)
        ...
        self.N_u = tf.placeholder(tf.float32, shape=[None, self.num_items], name='mask')



        #inference
        def generator():
            G_h1 = tf.nn.relu(tf.matmul(self.C, G_W1) + G_b1)
        ...
        self.D_loss = -tf.reduce_sum(tf.log(D_real+10e-5) + tf.log(1. - D_fake+10e-5))
        self.G_loss = tf.reduce_sum(tf.log(1.-D_fake+10e-5))#+self.alpha*tf.nn.l2_loss(tf.multiply(self.N_u,G_sample)))
